...
function brazilHolidays(year) {
  ...
  const easter = easterDate(year);
  const shift = (n) => { const r = new Date(easter); r.setDate(r.getDate() + n); return r.toISOString().slice(0, 10); };
  _holidayCache[year] = new Set([...fixed, shift(-2), shift(60)]);
  return _holidayCache[year];
}
...
function bizDaysInRange(start, end) {
  ...
  while (d <= e) {
    const dow = d.getDay();
    if (dow !== 0 && dow !== 6 && !brazilHolidays(d.getFullYear()).has(d.toISOString().slice(0, 10))) count++;
